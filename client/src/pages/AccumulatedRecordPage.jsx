...
function AccumulatedRecordPage() {
  ...
  const handleDownloadStudentExcel = () => {
    ...
    const data = studentRecords
      ...
      .map(r => ({ '날짜': r.date, '구분': r.type === 'text' ? '누가기록' : r.type === 'photo' ? '사진' : '동영상', '내용': r.content, '파일명': r.fileName || '-' }));
  ...
  const handleDownloadClassExcel = async () => {
    ...
    try {
      ...
      const data = filtered.map(r => {
        ...
        return { '번호': s?.student_number || '-', '이름': s?.name || '-', '날짜': r.date, '구분': r.type === 'text' ? '누가기록' : r.type === 'photo' ? '사진' : '동영상', '내용': r.content };
      });
      const worksheet = XLSX.utils.json_to_sheet(data);
      const workbook = XLSX.utils.book_new();
